<tr class="row-{{ $item->id }}"  @if(isset($loop))data-index="{{$loop->iteration}}"@endif>
    ...
    <td class="text-center align-middle">{{ date('d.m.Y', strtotime($item->start_date ?? "")) }}</td>
    <td class="text-center align-middle">{{ $item->end_date ? date('d.m.Y', strtotime($item->end_date)) : '-' }}</td>
